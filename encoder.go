...
// --------------------------------------------------------------
// Encoder implements the potato packing, similar to FC_BUFFER
// --------------------------------------------------------------
type Encoder struct {
	output io.Writer
...
func (e *Encoder) writeName(name Name) error {
	...
	if loggingEnabled {
		encoderLog.Debug("write Name", zap.Stringer("hex", HexBytes(val)))
	}
	if len(val) == 0 {
		return e.toWriter(bytes.Repeat([]byte{0}, 32))
	}
	return e.toWriter(val)
}
