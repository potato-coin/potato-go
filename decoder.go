...
// Decoder implements the potato unpacking, similar to FC_BUFFER
type Decoder struct {
	data               []byte
	pos                int
...
func (d *Decoder) decodeStruct(v interface{}, t reflect.Type, rv reflect.Value) (err error) {
	...
	for i := 0; i < l; i++ {
		...
		if tag != "binary_extension" && seenBinaryExtensionField {
			panic(fmt.Sprintf("the `potato: \"binary_extension\"` tags must be packed together at the end of struct fields, problematic field %s", typeField.Name))
		}
...
func (d *Decoder) ReadName() (out []byte, err error) {
	if d.remaining() < 32 {
		err = fmt.Errorf("Name required [%d] bytes, remaining [%d]", 32, d.remaining())
		return
	}
	...
	d.pos += 32
	decoderLog.Debug("read Name", zap.Stringer("hex", HexBytes(out)))
	return
}
