...
//			From:     AN("abourget"),
//			To:       AN("potato"),
//			Quantity: potato.Asset{Amount: 123123, Symbol: potato.potatoSymbol},
//		},
//	}
//
//	buf, err := potato.MarshalBinary(a)
...
//	assert.NoError(t, err)
//	assert.Equal(t, `{"account":"potato","authorization":[{"actor":"potato","permission":"active"}],"data":"00000059b1abe9310000000000ea3055f3e001000000000004454f530000000000","name":"transfer"}`, string(buf))
//
...
func TestUnpackActionTransfer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			Transfer{AN("tbcox2.3"), AN("tbcox2"), potato.Asset{Amount: 9}, ""},
		},
		{
			"00000003884ed1c900000000884ed1c90900000000000000000000000000000004616c6c6f",
			Transfer{AN("tbcox2.3"), AN("tbcox2"), potato.Asset{Amount: 9}, "allo"},
		},
	}
	...
	for idx, test := range tests {
		...
		var res Transfer
		assert.NoError(t, potato.UnmarshalBinary(buf, &res), fmt.Sprintf("Index %d", idx))
